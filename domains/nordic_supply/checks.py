...
def run(con, manifest, check, q, one):
    ...
    check("product names are unique", one("SELECT COUNT(*) - COUNT(DISTINCT name) FROM products") == 0)
    ...
    report["summaries"]["distributions"] = {
        ...
        "customers_without_orders": one("SELECT COUNT(*) FROM customers c WHERE NOT EXISTS (SELECT 1 FROM orders o WHERE o.customer_id = c.id)"),
